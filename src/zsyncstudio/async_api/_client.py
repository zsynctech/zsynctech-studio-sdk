"""Cliente assíncrono para a API do ZSync Tech Studio (mesma cobertura de `sync_api`)."""
...
class Client:
    """Equivalente assíncrono de `zsyncstudio.sync_api.Client`, para robôs baseados em asyncio.
    ...
    não pedem para você passá-lo de novo.

    ```python
    from zsyncstudio.async_api import Client

    async with Client(base_url, api_token) as client:
        execution = await client.get_pending_execution()
    ```
    ...
    async def start_execution(self) -> ExecutionRun:
        ...
        """
        data = await self._request("POST", "/executions", json={"instanceId": self.instance_id})
        return self.run_execution(Execution.from_api(data))

    async def schedule_execution(self) -> ExecutionRun:
        ...
        data = await self._request(
            "POST", "/executions/schedule", json={"instanceId": self.instance_id}
        )
        return self.run_execution(Execution.from_api(data))

    async def get_pending_execution(self, *, timeout: int = 20) -> Execution | None:
    ...
    async def poll_pending_executions(
        ...
        **não os relance** — uma exceção não capturada escapa do `while` e
        ...
        while True:
            try:
                pending = await self.get_pending_execution(timeout=timeout)
    ...
    async def claim_execution(self, execution_id: str) -> Execution:
        """Reivindica uma execução pendente, movendo-a de `PENDING` para `RUNNING`."""
        data = await self._request("POST", f"/executions/{execution_id}/claim")
    ...
    async def finish_execution(
        ...
        *,
        ...
        data = await self._request("POST", f"/executions/{execution_id}/finish", json=payload)
        return Execution.from_api(data)

    async def cancel_execution(self, execution_id: str) -> Execution:
        """Cancela uma execução `PENDING` ou `RUNNING`."""
        data = await self._request("POST", f"/executions/{execution_id}/cancel")
        return Execution.from_api(data)

    async def update_observation(self, execution_id: str, observation: str) -> Execution:
    ...
    async def set_total_tasks(self, execution_id: str, total_tasks: int) -> Execution:
    ...
    async def batch_complete_tasks(
        self, execution_id: str, tasks: list[TaskCompletion]
    ) -> list[Task]:
    ...
    async def get_task_summary(self, execution_id: str) -> TaskSummary:
        """Retorna contagens por status e métricas de tempo das tasks de uma execução."""
        data = await self._request("GET", f"/executions/{execution_id}/tasks/summary")
    ...
    def run_execution(self, execution: Execution | str) -> ExecutionRun:
        ...
        ```python
        pending = await client.get_pending_execution()
        if pending is not None:
            ...
            for invoice in invoices:
                ...
                try:
                    ...
                    await task.warning(str(warn))
                except TaskSkipped as skip:
                    await task.skip(str(skip))
                except Exception as exc:
            ...
            if execution.had_errors:
                await execution.error("alguns itens falharam")
            else:
    ...
    async def get_secret(self, secret_id: str, *, version: int | None = None) -> Secret:
        ...
        try:
            data = await self._request(
                "GET", f"/secrets/{secret_id}/reveal", params={"version": version}
            )
    ...
    async def rotate_secret(
        ...
        data = await self._request("POST", f"/secrets/{secret_id}/versions", json=payload)
        return SecretMeta.from_api(data)

    async def get_secret_status(self, secret_id: str) -> SecretMeta:
        ...
        """
        data = await self._request("GET", f"/secrets/{secret_id}")
        return SecretMeta.from_api(data)

    async def block_secret(self, secret_id: str, reason: str) -> SecretMeta:
        ...
        """
        return await self._set_secret_status(secret_id, SecretStatus.BLOCKED, reason)

    async def expire_secret(self, secret_id: str, reason: str) -> SecretMeta:
        ...
        """
        return await self._set_secret_status(secret_id, SecretStatus.EXPIRED, reason)

    async def _set_secret_status(
        self, secret_id: str, status: SecretStatus, reason: str
    ) -> SecretMeta:
        data = await self._request(
...
class ExecutionRun:
    """Equivalente assíncrono de `zsyncstudio.sync_api.ExecutionRun`."""
    ...
    async def start(self) -> Execution:
        ...
        `task()`.
        """
        return await self._client.claim_execution(self._execution_id)

    def task(self, reference: str) -> TaskRun:
        """Abre uma task local para `reference`.

        Nada é enviado à API até você chamar `await task.finish()`
        (`SUCCESS`), `task.warning(...)`, `task.error(...)` ou
        `task.skip(...)`. Chame `task.start()` antes de processar o item para
        que o `startedAt` reportado reflita o início real do processamento.
        """
        return TaskRun(self, reference)

    async def finish(self, observation: str | None = None) -> Execution:
        """Finaliza a execução como `COMPLETED`."""
        return await self._client.finish_execution(
            self._execution_id, observation=observation, status=ExecutionStatus.COMPLETED
        )

    async def error(self, observation: str | None = None) -> Execution:
    ...
    async def cancel(self) -> Execution:
        ...
        return await self._client.cancel_execution(self._execution_id)

    async def update_observation(self, observation: str) -> Execution:
        """Atualiza a observação da execução sem finalizá-la (segue `RUNNING`).

        Útil para reportar progresso (ex.: "processando lote 3 de 10")
        enquanto o loop de tasks ainda está rodando.
        """
        return await self._client.update_observation(self._execution_id, observation)

    async def set_total_tasks(self, total_tasks: int) -> Execution:
        ...
        de passar de 100%.
        """
        return await self._client.set_total_tasks(self._execution_id, total_tasks)
...
class TaskRun:
    ...
    async def finish(
        ...
        return await self._complete(TaskStatus.SUCCESS, observation, metadata)

    async def warning(
        self, observation: str | None = None, *, metadata: dict[str, Any] | None = None
    ) -> Task:
        """Reporta a task como `WARNING`."""
        return await self._complete(TaskStatus.WARNING, observation, metadata)

    async def error(
        self, observation: str | None = None, *, metadata: dict[str, Any] | None = None
    ) -> Task:
        """Reporta a task como `ERROR`."""
        return await self._complete(TaskStatus.ERROR, observation, metadata)

    async def skip(
        self, observation: str | None = None, *, metadata: dict[str, Any] | None = None
    ) -> Task:
        """Reporta a task como `SKIPPED`."""
        return await self._complete(TaskStatus.SKIPPED, observation, metadata)
    ...
    async def _complete(
        ...
        return await self._run._client.complete_task(
            self._run.id,
            self.reference,
            status,
            observation=observation,
...
class Secret:
    """Equivalente assíncrono de `zsyncstudio.sync_api.Secret`."""
    ...
    async def rotate(self, value: Any, *, expires_at: datetime | None = None) -> SecretMeta:
        ...
        recém-rotacionado.
        """
        return await self._client.rotate_secret(self.secret_id, value, expires_at=expires_at)

    async def block(self, reason: str) -> SecretMeta:
        """Marca esta credencial como `BLOCKED`. Equivalente a
        `client.block_secret(secret.secret_id, reason)`.
        """
        return await self._client.block_secret(self.secret_id, reason)

    async def expire(self, reason: str) -> SecretMeta:
        """Marca esta credencial como `EXPIRED`. Equivalente a
        `client.expire_secret(secret.secret_id, reason)`.
        """
        return await self._client.expire_secret(self.secret_id, reason)
